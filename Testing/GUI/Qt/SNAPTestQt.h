...
public slots:
  ...
  // Invoke any method the wrapped object's meta-object declares, matched by
  // name and argument count, with each argument converted to the declared
  // parameter type. A proxy among the arguments is unwrapped to the object it
  // stands for, so a script can hand one widget to a method of another
  // (e.g. QTabWidget::setCurrentWidget). Posted like every other action.
  // Returns false if the object declares no such method.
  bool callMethod(QString method, QVariantList args = QVariantList());
...
public slots:
  ...
  // Engine-level access to a wrapped object's properties and methods. These
  // are the entry points the test scripts use (engine.setChildProperty(...),
  // engine.clickChild(...), ...); each resolves to a TestObjectProxy member,
  // so it marshals onto the GUI thread exactly like a direct call on the proxy
  // would. A null object is a script error, not a silent no-op.
  //
  // `block` is accepted for script compatibility and has no effect: every
  // action already round-trips through the GUI thread's event queue before
  // returning (see TestObjectProxy::syncWithGuiThread), and that barrier is
  // safe against a nested modal loop, so there is nothing to opt into.
  QVariant getProperty(TestObjectProxy *obj, QString name);
  void setProperty(TestObjectProxy *obj, QString name, QVariant value);
  void callMethod(TestObjectProxy *obj, QString method,
                  QVariantList args = QVariantList(), bool block = false);

  // Same, but for a child found by name (avoids a throwaway findChild var)
  QVariant getChildProperty(TestObjectProxy *parent, QString childName, QString propName);
  void setChildProperty(TestObjectProxy *parent, QString childName, QString propName, QVariant value);
  void callChildMethod(TestObjectProxy *parent, QString childName, QString method,
                       QVariantList args = QVariantList(), bool block = false);

  // click/toggle/close are just callMethod for the most commonly called slots
  void click(TestObjectProxy *obj, bool block = false);
  void clickChild(TestObjectProxy *parent, QString childName, bool block = false);
  void toggle(TestObjectProxy *obj, bool block = false);
  void close(TestObjectProxy *obj, bool block = false);
  void closeChild(TestObjectProxy *parent, QString childName, bool block = false);

  // validateProperty/validateChildProperty fold a getProperty into validateValue
  void validateProperty(TestObjectProxy *obj, QString name, QVariant expected, double precision = -1.0);
  void validateChildProperty(TestObjectProxy *parent, QString childName, QString propName,
